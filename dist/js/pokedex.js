...
const populateResults = (newFirstPageNum = 1, newActivePageNum = 1) => {
  const pokeSearchResults = document.querySelector('#pokeSearchResults');
  while (pokeSearchResults.firstElementChild) {
    pokeSearchResults.firstElementChild.remove();
  }

  const results = getResults();
  const startIndex = POKE_PER_PAGE * (newActivePageNum - 1);
...
const createFilterBtn = (searchQuery) => {
  ...
  //   <button class="btn btn-dark filter-tag">
  //     "${searchQuery}" <i class="fas fa-times-circle fa-xs"></i>
  //   </button>`;
...
const removeFilter = (event) => {
  ...
  document.querySelector('#filterBtn').classList.add('d-none');
...
const createPokemonCard = (pokemon) => {
  ...
  const pokeSearchResults = document.querySelector('#pokeSearchResults');
  pokeSearchResults.appendChild(pokemonCard);
};
...
const createPagination = (
  ...
  if (results.length <= POKE_PER_PAGE) {
    ...
    // });
    pagination.addEventListener('click', displayNewPage);
  }
};
